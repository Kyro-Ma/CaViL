...
# Or with custom params: LR=1e-4 NUM_EPOCHS=3 ./run_prompt.sh Baby_Products

# Configuration
# Base HF model name/path (used when loading a .ckpt)
BASE_MODEL_NAME="${BASE_MODEL_NAME:-llava-hf/llava-v1.6-mistral-7b-hf}"

# Distillation output root (contains per-category adapter dirs)
DISTILLED_ROOT="${DISTILLED_ROOT:-./fly_out_distilled}"
...
if [ "$#" -eq 0 ]; then
    ...
    echo "Usage: ./run_prompt.sh <category1> [category2] ..."
    echo "Example: ./run_prompt.sh Baby_Products"
    exit 1
fi
...
for category in "$@"; do
    ...
    OUTPUT_DIR="./fly_out_finetuned/${category}"
    mkdir -p "${OUTPUT_DIR}"

    # Prefer adapter dir, fallback to latest .ckpt
    ADAPTER_DIR="${DISTILLED_ROOT}/${category}/vision_lora_adapter_best"
    CKPT_PATH="$(ls -1t "${DISTILLED_ROOT}/${category}"/*.ckpt 2>/dev/null | head -n 1 || true)"

    USE_CKPT="false"
    if [ -d "${ADAPTER_DIR}" ]; then
        USE_CKPT="false"
    elif [ -n "${CKPT_PATH}" ] && [ -f "${CKPT_PATH}" ]; then
        USE_CKPT="true"
    else
        echo "[ERROR] No adapter dir or .ckpt found under: ${DISTILLED_ROOT}/${category}/"
        echo "Existing files:"
        ls -lah "${DISTILLED_ROOT}/${category}" || true
        exit 1
    fi
    ...
    # Build command
    CMD="python3 fly_prompt_tuning.py"
    # CMD="CUDA_VISIBLE_DEVICES=1 python3 fly_prompt_tuning.py"
    CMD="${CMD} --category '${category}'"
    CMD="${CMD} --finetune_output_dir '${OUTPUT_DIR}'"
    if [ "${USE_CKPT}" = "true" ]; then
        CMD="${CMD} --model_dir '${BASE_MODEL_NAME}'"
        CMD="${CMD} --checkpoint_path '${CKPT_PATH}'"
    else
        CMD="${CMD} --model_dir '${ADAPTER_DIR}'"
    fi
